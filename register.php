...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['register'])) {
        ...
        // makay9belch ila had les forms:(accepte @gmail.com, @hotmail.com, et d'autres formats)
        if (!preg_match("/^[\w\-\.]+@(gmail\.com|hotmail\.com|yahoo\.com|outlook\.com)$/", $email)) {
            $error = "Format d'email invalide. Seuls les emails Gmail, Hotmail, Yahoo et Outlook sont autorisés.";
        }
        // darori pwd ykon fih:(au moins 8 caractères, une majuscule, et un caractère spécial)
        elseif (!preg_match("/^(?=.*[A-Z])(?=.*[!@#$%^&*()_+\-=\[\]{};':\"\\|,.<>\/?])(?=.{8,})/", $password_plain)) {
            $error = "Le mot de passe doit contenir au moins 8 caractères, une lettre majuscule et un caractère spécial.";
        } else {
